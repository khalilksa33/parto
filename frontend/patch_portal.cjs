...
// Add dispatches state
content = content.replace(
  `const [orders, setOrders] = useState<any[]>([]);`,
  `const [orders, setOrders] = useState<any[]>([]);\n  const [dispatches, setDispatches] = useState<any[]>([]);`
);

// Fetch dispatches in useEffect
...
const fetchDispatchesStr = `
        ...
        // If tow_company, fetch dispatches
...
const acceptDispatchStr = `
  const handleLogout = () => {
`;
const acceptDispatchImpl = `
  ...
  const handleLogout = () => {
`;
content = content.replace(acceptDispatchStr, acceptDispatchImpl);


// Change dashboard tab names for tow_company
const tabsStr = `
            {/* Dashboard Tabs */}
...
// Change order view to show dispatches if tow_company
const ordersTabStr = `
                    ...
                    {dispatches.map(dispatch => (
                      <div key={dispatch.id} className="bg-slate-900/40 border border-slate-800 rounded-2xl p-4 flex flex-col gap-3">
                        ...
                        {dispatch.status === 'pending' && (
                          <button onClick={() => handleAcceptDispatch(dispatch.id)} className="w-full mt-2 bg-indigo-650 hover:bg-indigo-600 text-white font-bold py-2 rounded-xl text-xs transition-colors">
                            Accept Dispatch
                          </button>
                        )}
...
content = content.replace(ordersTabStr, dispatchesTabStr);

fs.writeFileSync('src/app/[locale]/portal/page.tsx', content);
console.log('Modified portal/page.tsx');
